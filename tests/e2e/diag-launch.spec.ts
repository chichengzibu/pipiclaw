 ...
 *
 * 用途: 排查 Electron 启动期错误(主进程 / preload / 渲染前段)
 *
 * 用法:
 *   E2E_DIAG=1 npx playwright test tests/e2e/diag-launch.spec.ts
 *
 * P1-T1.1: 加 E2E_DIAG 守卫,默认 skip 避免 CI 误触
 */

const repoRoot = path.resolve(__dirname, '..', '..')
const mainEntry = path.join(repoRoot, 'dist-electron', 'main.js')
const shouldRunDiag = !!process.env.E2E_DIAG

test('diagnose electron launch', () => {
  test.skip(!shouldRunDiag, 'diagnostic test — set E2E_DIAG=1 to run')
  ...
  return (async () => {
    ...
    app.process().stdout?.on('data', (d) =>
      ...
      console.log(`[main stdout] ${d.toString().trim()}`),
    )
    app.process().stderr?.on('data', (d) =>
       
      console.log(`[main stderr] ${d.toString().trim()}`),
    )
    ...
    app.on('window', async (window) => {
      ...
      const html = await window
        .evaluate(() => document.getElementById('app')?.innerHTML ?? '<no #app>')
