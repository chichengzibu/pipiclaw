...
test.describe('T+60 设置 → 模型管理 CRUD', () => {
  ...
  test('M2: 切到模型管理 tab', async ({ window }) => {
    ...
    const active = await window.locator('.el-tabs__item.is-active').first().textContent()
    expect(active).toMatch(/模型管理|Models/i)
  })

  test('M3: 点"添加 Provider"按钮 → 打开对话框', async ({ window }) => {
    await window.click('a.nav-item[href$="#/settings"]')
    await window.waitForURL(/#\/settings/)
    const modelsTab = window.locator('.el-tabs__item:has-text("模型管理"), .el-tabs__item:has-text("Models")').first()
    await modelsTab.click()
    await window.waitForTimeout(500)
    const addBtn = window.locator('button:has-text("添加 Provider"), button:has-text("Add Provider")').first()
    await addBtn.click()
    await window.waitForTimeout(500)
    // 对话框标题(中英文) — Element Plus dialog 用 .el-dialog__wrapper 包裹,visible 的 wrapper 才计 1
    const dialogTitle = window.locator('.el-dialog__title:has-text("添加 Provider"), .el-dialog__title:has-text("Add Provider")')
  ...
  test('M5: 必填项为空时 → 提交按钮报错(或 dialog 不关闭)', async ({ window }) => {
    ...
    // dialog 应该仍然 visible
    const dialogVisible = await window.locator('.el-dialog:visible .el-dialog__title:has-text("添加 Provider"), .el-dialog:visible .el-dialog__title:has-text("Add Provider")').count()
  ...
  test('M6: 关闭对话框 → 取消按钮工作', async ({ window }) => {
    ...
    // dialog 应关闭
    const dialogCount = await window.locator('.el-dialog:visible .el-dialog__title:has-text("添加 Provider"), .el-dialog:visible .el-dialog__title:has-text("Add Provider")').count()
