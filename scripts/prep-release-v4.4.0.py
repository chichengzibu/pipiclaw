...
# -*- coding: utf-8 -*-
"""
Prep v4.4.0 release body (UTF-8 no BOM) + tag commit message.
...
release_body = """## v4.4.0 — P0 安全 + v4.4 视觉精简 (团队协作首秀)
...
body_path = os.path.join(repo, 'release-body-v4.4.0.md')
...
# 3) Write release JSON payload (UTF-8 no BOM)
payload = {
    'tag_name': 'v4.4.0',
    'name': 'PiPiClaw v4.4.0 — P0 安全 + v4.4 视觉精简 (团队协作首秀)',
    'body': release_body,
    'draft': False,
    'prerelease': False,
    'target_commitish': 'master',
}
payload_path = os.path.join(repo, 'release-payload-v4.4.0.json')
...
print()
print('--- next steps ---')
print('1) wait for npm run build to finish (bg_b8aba6c1)')
print('2) verify Setup.exe exists in dist/')
print('3) commit package.json version bump')
print('4) git tag v4.4.0')
print('5) git push origin master v4.4.0')
print('6) curl POST release payload to GitHub API')
