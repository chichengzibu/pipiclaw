...
const OUT = 'ui-screenshots-real-user-3'
...
async function main() {
  ...
  try {
    ...
    try {
      // 0) 直接看 IPC 返什么
      const directActive = await win.evaluate(async () => {
        return await (window).electronAPI.permissions.active?.() || (window).electronAPI.permissions?.getActive?.() || null
      })
      console.log('  IPC permissions.active 返:', JSON.stringify(directActive).slice(0, 200))
    ...
    try {
      ...
      for (let i = 0; i < turns.length; i++) {
        ...
        // 1) 改用 JS 直接 fill + dispatchEvent 触发 v-model
        await win.fill(inputSel, prompt)
        // 2) 用 dispatchEvent 模拟 Enter (element-plus textarea 兼容)
        await win.evaluate(() => {
        ...
        const sentAfterKeydown = await win.evaluate(() => {
          return document.querySelectorAll('.chat-message, [class*="message"]').length
        })
        if (sentAfterKeydown < (i + 1) * 2) {
          console.log(`  [fallback] 尝试点击发送按钮...`)
          await win.evaluate(() => {
            const btns = document.querySelectorAll('button')
            for (const b of btns) {
              const txt = b.textContent?.trim() || ''
              if (txt.includes('发送') || txt.includes('Send')) { b.click(); return }
            }
          })
        }
        ...
        if (chatLen < (i + 1) * 2) {
          record('p1', `Chat 轮 ${i+1}`, `消息数 ${chatLen} 少于预期 (input 填了但 send 没触发)`)
        }
